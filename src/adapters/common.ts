...
function getElementTextHint(el: HTMLElement): string {
  return [
...
function scoreInput(el: HTMLElement): number {
  ...
  if (el instanceof HTMLTextAreaElement) score += 8;
  if (el.getAttribute('contenteditable') === 'true') score += 6;
  if (el.getAttribute('role') === 'textbox') score += 4;
  if (!el.hasAttribute('readonly') && !(el as HTMLTextAreaElement).disabled) score += 2;
  if (el.closest('form')) score += 3;
  if (el.closest('[role="main"]')) score += 3;
...
export function findGenericChatInput(root: ParentNode = document): HTMLTextAreaElement | HTMLElement | null {
  const candidates = Array.from(root.querySelectorAll<HTMLElement>(INPUT_SELECTORS)).filter(isVisible);
...
export function findGenericMountTarget(
  ...
  for (const selector of selectors) {
    const container = inputEl.closest<HTMLElement>(selector);
    if (container) return { container, append: true };
  }
...
export function readGenericThemeMode(): ThemeMode | null {
