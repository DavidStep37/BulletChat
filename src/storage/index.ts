import type { SuggestionItem, StoredSuggestions } from '../types/index.js';
...
export async function getSuggestions(): Promise<SuggestionItem[]> {
  ...
  const data = raw[STORAGE_KEY] as StoredSuggestions | undefined;
  if (data?.items?.length) return data.items;
  return DEFAULT_SUGGESTIONS;
}

export async function setSuggestions(items: SuggestionItem[]): Promise<void> {
